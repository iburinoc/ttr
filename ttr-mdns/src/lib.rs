...
impl Server {
    fn parse(service_name: &str, txt: &[u8], address: SocketAddr) -> Result<Self, MdnsError> {
        ...
        let get_int = |name| {
            str::from_utf8(get(name)?)
                .map_err(|_| inv_txt())?
                .parse()
                .map_err(|_| inv_txt())
        };
        let get_str = |name| String::from_utf8(get(name)?).map_err(|_| inv_txt());

        let opaque = get_int("opaque")?;
        let game_status = get_int("gameStatus")?;
        let name = get_str("_d")?;
        let uuid = str::from_utf8(get(name)?).parse().map_err(|_| inv_txt())?;
...
pub async fn browse(timeout: Option<Duration>) -> Result<Option<Server>, DnssdError> {
    let timeout = timeout.unwrap_or(Duration::from_secs(3600 * 24 * 365 * 20));
    let browse = async_dnssd::browse(MDNS_TYPE)?.timeout(timeout)?;
    let stream = browse.try_filter_map(|service| async move {
